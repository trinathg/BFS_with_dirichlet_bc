...
const int file_freq=10; // Frequency of writing down files for data visualization
...
const double l_x = 40.0; //Non dimensional units 
...
const double lambda = -(Re/2.)*( sqrt(1. + (16.*M_PI*M_PI/(Re*Re))) - 1.0 ); 

const double dx = l_x/nx; //The grid spacing in X and Y-directions 
const double dy = l_y/ny; 

const double delta = 0.01; //The small parameter in the outflow BC from Dong's paper. 

const double dt = 1.0e-4; //CFL_d*(dx)*(dx)*Re; // 1/Re acts as a diffusion coefficient here 

const double beta_cfl = 0.2;	
